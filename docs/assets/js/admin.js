...
async function loadOverview() {
  const [{ count: pc }, { count: nc }, { count: sc }, { data: settings }] = await Promise.all([
    ...
    supabase.from("settings").select("visitor_count").eq("id", 1).single(),
  ]);
...
async function loadAboutForm() {
  const { data: a } = await supabase.from("about").select("*").eq("id", 1).single();
  ...
  const saveAbout = async () => {
    ...
    const { error } = await supabase.from("about").upsert({ id: 1, ...CURRENT_ABOUT, updated_at: new Date().toISOString() });
...
async function loadResumeForm() {
  const { data: p } = await supabase.from("profile").select("resume_url").eq("id", 1).single();
  ...
  document.getElementById("resumeDrop").addEventListener("click", () => {
    setTimeout(async () => {
      ...
      await supabase.from("profile").upsert({ id: 1, resume_url: hidden.value, updated_at: new Date().toISOString() });
    }, 1200);
  });
}
...
function openProjectModal(existing) {
  ...
  document.getElementById("m_save").addEventListener("click", async () => {
    ...
    let error;
    if (existing) ({ error } = await supabase.from("projects").update(payload).eq("id", existing.id));
...
function openSkillModal(existing) {
  ...
  document.getElementById("m_save").addEventListener("click", async () => {
    ...
    let error;
    if (existing) ({ error } = await supabase.from("skills").update(payload).eq("id", existing.id));
...
function openCertModal(existing) {
  ...
  document.getElementById("m_save").addEventListener("click", async () => {
    ...
    let error;
    if (existing) ({ error } = await supabase.from("certificates").update(payload).eq("id", existing.id));
...
function openNoteModal(existing) {
  ...
  document.getElementById("m_save").addEventListener("click", async () => {
    ...
    let error;
    if (existing) ({ error } = await supabase.from("notes").update(payload).eq("id", existing.id));
...
async function loadLearningTable() {
  const { data } = await supabase.from("learning").select("*").order("sort_order");
...
function openLearningModal(existing) {
  ...
  document.getElementById("m_save").addEventListener("click", async () => {
    ...
    let error;
    if (existing) ({ error } = await supabase.from("learning").update(payload).eq("id", existing.id));
    else ({ error } = await supabase.from("learning").insert(payload));
    if (error) { toast("Save failed: " + error.message, "err"); return; }
  ...
  document.getElementById("addGalleryBtn").addEventListener("click", () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        const { error } = await supabase.from("gallery").insert({ image_url: url, caption, category: "General" });
