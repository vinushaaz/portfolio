...
async function bumpVisitorCount() {
  try { await supabase.rpc("increment_visitor_count"); } catch (e) { /* function optional */ }
}
...
async function loadProfile() {
  ...
  socialsHost.innerHTML = socials.filter(s => p[s.key]).map(s =>
...
async function loadProjects() {
  const { data } = await supabase.from("projects").select("*").order("featured", { ascending: false }).order("sort_order");
  ALL_PROJECTS = data || [];
...
async function loadNotes() {
  const { data } = await supabase.from("notes").select("*").eq("published", true).order("created_at", { ascending: false });
  ALL_NOTES = data || [];
